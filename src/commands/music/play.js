    ...
    try {
      ...
      if (ytdl.validateURL(input)) {
        ...
        if (youtubeService.isMixPlaylist(input)) {
          message.reply(
            "Starting playback. Loading more songs in background..."
          );

          // Immediately get and play first song
          const firstSong = await youtubeService.getVideoInfo(url);
          const songToAdd = {
            ...firstSong,
          ...
          if (!queue.playing) {
            queue.playing = true;
            await musicPlayer.playSong(message.guild.id, songToAdd);
          }

          // Load rest of mix in background
          this.loadMixInBackground(message, url);
        } else {
